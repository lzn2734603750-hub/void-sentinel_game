...
function isAnyPlayerAlive() {
    for (var i = 0; i < players.length; i++) {
        if (players[i] && players[i].alive && players[i].hp > 0) return true;
    }
    return false;
}

function findNearestAlivePlayer(x, y) {
    var nearest = null;
...
function applyEnemyMovementAI(e, targetPlayer) {
...
function updateEnemiesGuestVisual() {
    for (var i = enemies.length - 1; i >= 0; i--) {
        ...
        if (!isAnyPlayerAlive()) continue;

        e.wobble += 0.05;

        var targetPlayer = findNearestAlivePlayer(e.x, e.y);
        if (!targetPlayer) continue;

        applyEnemyMovementAI(e, targetPlayer);
    }
}
...
function updateBoss() {
    ...
    var targetPlayer = findNearestAlivePlayer(boss.x, boss.y);
...
function drawBoss() {
    ...
    var eyePlayer = findNearestAlivePlayer(boss.x, boss.y);
